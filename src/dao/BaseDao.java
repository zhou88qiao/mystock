...
public class BaseDao {
		...
		try {
			...
			while (rs.next()) {
				for (int i = 0; i < colLen; i++) {
					...
					/*
					 * String cols_name = metaData.getColumnName(i + 1); Object
					 * cols_value = rs.getObject(cols_name); if (cols_value ==
					 * null)// 列的值没有时，设置列值为“” { cols_value = ""; }
	...
	/*
	 * public <T> T executeSingleQuery(String sql, Class<T>cls, List<Object>
	 * params) throws SQLException, InstantiationException,
	 * IllegalAccessException, SecurityException, NoSuchFieldException { T
	 * resultObject = null; int index = 1; ps = conn.prepareStatement(sql);
	 * if(params != null && !params.isEmpty()){ for(int i = 0; i<params.size();
	 * i++){ ps.setObject(index++, params.get(i)); } } rs = ps.executeQuery();
	 * ResultSetMetaData metaData = rs.getMetaData(); int cols_len =
	 * metaData.getColumnCount(); while(rs.next()) { //通过反射机制创建一个实例 resultObject
	 * = (T) cls.newInstance(); for(int i = 0; i<cols_len; i++){ String
	 * cols_name = metaData.getColumnName(i+1); Object cols_value =
	 * rs.getObject(cols_name); if(cols_value == null){ cols_value = ""; } Field
	 * field = cls.getDeclaredField(cols_name); field.setAccessible(true);
	 * //打开javabean的访问权限 field.set(resultObject, cols_value); } } return
	 * resultObject;
	 * 
	 * }
	 */
	 ...
	 *            要保存或更新的实体对象
	 * @param args
	 *            不需要赋值的列标组成的数组，例如sql语句 "insert into tbl_user
	 *            values(seq_user.nextval,?,?,?)"应为1
	 ...
	 * @throws SQLException
	 */
	public <T> T executeSingleQuery(String sql, Class<T> clazz, Object... args)
			throws IOException, ClassNotFoundException, SQLException {
		// Connection connTmp = DbConn.getConn();
		T object = null;
		try {
			ps = conn.prepareStatement(sql);
			for (int i = 0; i < args.length; i++) {
			...
			DbConn.closeResult(ps, rs);
			// DbConn.closeConn(connTmp, ps, rs);
		}
		return null;
	}
